...
<style>
    ...
    .button--transparent-rounded:hover {
        ...
        color: #000;
        box-shadow: 0 4px 15px rgba(239, 193, 39, 0.35);
    ...
    .tags__item:hover {
        background: rgba(239, 193, 39, 0.12);
        border-color: rgba(239, 193, 39, 0.35);
        color: var(--accent-color);
    ...
    .formats__item:hover .formats__box {
        border-color: rgba(239, 193, 39, 0.3);
        background: rgba(239, 193, 39, 0.03);
    ...
    .sv-toast {
        ...
        font-size: 13.5px;
        z-index: 99999;
        box-shadow: 0 10px 30px rgba(239,193,39,0.35);
        opacity: 0;
        pointer-events: none;
    ...
    .comments-wrapper input[type="email"]:focus {
        border-color: rgba(239, 193, 39, 0.45) !important;
        outline: none;
    }
    ...
    .comments-wrapper .submit:hover {
        ...
        transform: translateY(-2px);
        box-shadow: 0 4px 15px rgba(239, 193, 39, 0.3);
    }
                ...
                <div class="watch__main">
                    ...
                    <div class="player-wrap">
                        <div class="player-item__block">
                            ...
                            <!-- Initial Poster Display -->
                            <?php if (!empty($poster_url)) : ?>
                                <picture class="player-item__inner">
                                    <img src="<?php echo esc_url($poster_url); ?>" alt="<?php the_title_attribute(); ?>">
                                </picture>
                            <?php endif; ?>
                            
                            <button class="player-poster-play" aria-label="Play video" type="button"></button>
                            
                            <!-- Player elements (initially hidden) -->
                            <?php if (!empty($iframe_url)) : ?>
                                <iframe data-src="<?php echo esc_url($iframe_url); ?>" src="about:blank" frameborder="0" allowfullscreen style="display: none;"></iframe>
                            <?php elseif (!empty($video_url) && preg_match('/\.mp4$/i', $video_url)) : ?>
                                <video src="<?php echo esc_url($video_url); ?>" controls style="display: none; width:100%; height:100%; object-fit:contain;"></video>
                            <?php else : ?>
                                <div class="no-player-msg" style="position:absolute; top:0; left:0; width:100%; height:100%; z-index:2; background:#111; display:flex; flex-direction:column; align-items:center; justify-content:center; color:#50585e; display: none;">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Play Overlay button interaction
        var playBtn = document.querySelector('.player-poster-play');
        if (playBtn) {
            playBtn.addEventListener('click', function() {
                var wrap = this.closest('.player-item__block');
                if (wrap) {
                    var poster = wrap.querySelector('.player-item__inner');
                    var iframe = wrap.querySelector('iframe');
                    var video  = wrap.querySelector('video');
                    var noPlay = wrap.querySelector('.no-player-msg');
                    ...
                    if (iframe) {
                        var actualSrc = iframe.getAttribute('data-src');
                        if (actualSrc) {
                            if (actualSrc.indexOf('?') > -1) {
                                actualSrc += '&autoplay=1';
                            } else {
                                actualSrc += '?autoplay=1';
                            }
                            iframe.setAttribute('src', actualSrc);
                        }

...
function hexmy_scripts() {
    ...
    // Google Fonts
    wp_enqueue_style('hexmy-fonts', 'https://fonts.googleapis.com/css2?family=Archivo+Narrow:wght@400;500;600;700&family=Roboto:wght@400;500;700&display=swap', array(), null);
...
// Register custom rewrite rules and query variable for static Page categories pagination
function hexmy_register_custom_category_rewrite() {
    add_rewrite_rule('^category/page/([0-9]+)/?', 'index.php?pagename=category&cat_paged=$matches[1]', 'top');
    flush_rewrite_rules(false);
}
add_action('init', 'hexmy_register_custom_category_rewrite');

function hexmy_register_custom_category_query_vars($vars) {
    $vars[] = 'cat_paged';
    return $vars;
}
add_filter('query_vars', 'hexmy_register_custom_category_query_vars');

...
// Count total tag terms that are non-empty
$total_terms = wp_count_terms('video_tag', array('hide_empty' => true));
    ...
    <div class="container">
        ...
        <section class="section">
            ...
            <?php if (!empty($categories) && !is_wp_error($categories)) : 
                ...
                // Query a backup pool of recent videos to draw unique images from if a tag's own videos are exhausted
            ...
            ?>
                <div class="channels-grid">
                    <?php foreach ($categories as $cat) : 
                        ...
                        // Retrieve tag image url, checking if it was already used to avoid duplicate images
                        $image_url = '';
                        $cached_img = get_term_meta($cat->term_id, '_tag_image_url', true);
                        
                        // If cached image is empty, find the first available image from the tag's videos and cache it
                        if (empty($cached_img)) {
                            ...
                            if (!empty($tag_first_vids)) {
                                foreach ($tag_first_vids as $v) {
                                    ...
                                    if (!empty($v_img)) {
                                        $cached_img = $v_img;
                                        update_term_meta($cat->term_id, '_tag_image_url', $v_img);
                                        break;
                                    }
                                }
                            }
                        ...
                        if (!empty($cached_img) && !in_array($cached_img, $used_image_urls)) {
                            ...
                            // Query up to 100 videos of this tag to find a unique image URL
                        ...
                        // Retrieve associated video categories to display inside the card footer
                        ...
                        if (!empty($associated_videos)) {
                            foreach ($associated_videos as $vid_id) {
                                $vid_cats = get_the_terms($vid_id, 'video_category');
                                if ($vid_cats && !is_wp_error($vid_cats)) {
                                    foreach ($vid_cats as $c) {
                                        $unique_categories[$c->term_id] = $c;
                                    }
                                }
                            }
                        }
                        // Limit to top 2 associated categories
                        $display_cats = array_slice($unique_categories, 0, 2);
                    ...
                    ?>
                        <div class="category-item">
                            ...
                            <a href="<?php echo esc_url($term_link); ?>" class="category-item__circle-badge">
                                ...
                                <span class="circle-badge__sub">4K</span>
                            </a>
            ...
            <?php else : ?>
                <div class="no-categories-found">
                    <p>No video tags found. Tags present in video posts will display here.</p>
                </div>
            <?php endif; ?>
        </section>
